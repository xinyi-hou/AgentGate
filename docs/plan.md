...
1. **TS-Bench**：调用前语义判定；
2. **AgentDojo**：间接注入和端到端效用；
3. **自建AgentGateBench**：可控后端、信息流和消融回归。

为避免围绕单一 Benchmark 调规则，再增加四个彼此互补的外部泛化套件：

4. **InjecAgent**：1,054 条间接注入、直接危害和数据窃取案例；
5. **ToolEmu**：144 条案例、36 个工具包，评估工具副作用；
6. **tau2-bench**：航空、零售、电信和银行业务策略与正常任务效用；
7. **MCP-SafetyBench**：245 个任务、20 类 MCP 投毒、返回注入、重放和权限滥用攻击。

新增套件只有在完成统一安全 IR 适配后才能进入论文结果。适配器只负责格式转换，不能将
Benchmark 文本、实体名称、标签或攻击模板写入生产规则。固定版本、许可证、接入优先级和
反过拟合协议见 [benchmark-strategy.md](benchmark-strategy.md)。
...
## 11.3 扩展跨Benchmark泛化评估

四个扩展套件分别补足三个模块：

| Benchmark | 主要模块 | 主要作用 |
| --- | --- | --- |
| InjecAgent | 模块一、模块二 | 检查不同注入模板和工具组合下的上下文操控与数据窃取 |
| ToolEmu | 模块二、模块三 | 检查未见工具域中的真实效果、副作用和执行后状态 |
| tau2-bench | 模块二 | 检查业务政策、任务完成率和安全控制对正常效用的影响 |
| MCP-SafetyBench | 模块一、二、三 | 检查工具投毒、影子工具、返回注入、重放和多步权限滥用 |

评测前必须冻结 Prompt、风险字段、阈值和工具画像映射，并执行任务同义改写、工具重命名、
Schema 字段改名以及注入分隔符扰动。规则、语义抽取、证据融合和完整系统必须消费相同的
统一事件，不能为某个 Benchmark 单独放宽资源、效果或审批策略后再做横向比较。

---

## 11.4 自建AgentGateBench
